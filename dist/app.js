...
function metrics(p){var l=latest(p),rows=p.snapshots||[],i=+p.initialValue||0,c=+l.value||0,raw=+l.fees||0,baseline=p.feesBaseline!=null?+p.feesBaseline:0,f=Math.max(0,raw-baseline),reported=l.pnl!=null?+l.pnl:null,capital=p.source==="byreal"&&reported!=null?Math.max(0,c+f-reported):i,v=c-capital,pnl=reported!=null?reported:v+f,elapsed=days(p,l.capturedAt||l.date),apr=l.apr!=null?+l.apr:(p.source==="byreal"&&p.historyScope==="byreal-lifetime"&&capital&&elapsed>=.9?f/capital*365/elapsed*100:rows.length>1&&elapsed>=.9&&capital?f/capital*365/elapsed*100:null);return{initial:i,capital:capital,current:c,fees:f,variation:v,pnl:pnl,apr:apr,price:l.price}}
...
function card(p){
  ...
  '<div class="poolStats"><div class="poolStat"><span>Aporte inicial</span><strong>'+money.format(m.initial)+'</strong></div><div class="poolStat"><span title="Aporte inicial ajustado por depósitos e retiradas">Base ajustada</span><strong>'+money.format(m.capital)+'</strong></div><div class="poolStat"><span>Liquidez atual</span><strong>'+money.format(m.current)+'</strong></div><div class="poolStat"><span>'+feeLabel+'</span><strong class="positive">'+money.format(m.fees)+'</strong></div><div class="poolStat"><span title="Na Byreal, usa o PnL informado pela posição">'+pnlLabel+'</span><strong class="'+tone(m.pnl)+'">'+signed(m.pnl)+'</strong></div><div class="poolStat"><span title="Taxas acumuladas anualizadas pelo capital ajustado">'+aprLabel+'</span><strong>'+(m.apr==null?"—":num.format(m.apr)+"%")+'</strong></div></div>'+
...
function summary(){
  var items=app.data.pools.map(function(p){return{pool:p,metric:metrics(p)}}),scope=items.filter(function(x){return app.filter==="all"||x.pool.status===app.filter}).map(function(x){return x.metric}),capital=scope.reduce(function(a,m){return a+m.capital},0),current=scope.reduce(function(a,m){return a+m.current},0),fees=scope.reduce(function(a,m){return a+m.fees},0),variation=scope.reduce(function(a,m){return a+m.variation},0),pnl=scope.reduce(function(a,m){return a+m.pnl},0),eligible=scope.filter(function(m){return m.apr!=null}),eligibleCapital=eligible.reduce(function(a,m){return a+m.capital},0),apr=eligibleCapital?eligible.reduce(function(a,m){return a+m.apr*m.capital},0)/eligibleCapital:null,scopeName=app.filter==="active"?"ativas":app.filter==="closed"?"fechadas":"no total";
  $("#sumCapitalLabel").textContent=app.filter==="active"?"Capital ativo":app.filter==="closed"?"Base encerrada":"Base ajustada";$("#sumCurrentLabel").textContent=app.filter==="active"?"Liquidez atual":app.filter==="closed"?"Liquidez no encerramento":"Liquidez registrada";$("#sumInitial").textContent=money.format(capital);$("#sumCurrent").textContent=money.format(current);$("#sumFees").textContent=money.format(fees);$("#sumVariation").textContent=signed(variation);$("#sumVariation").className=tone(variation);$("#sumVariationPct").textContent=(variation>=0?"+":"")+num.format(capital?variation/capital*100:0)+"%";$("#sumPnl").textContent=signed(pnl);$("#sumPnl").className=tone(pnl);$("#sumPnlPct").textContent=(pnl>=0?"+":"")+num.format(capital?pnl/capital*100:0)+"%";$("#sumApr").textContent=apr==null?"—":num.format(apr)+"%";$("#poolCount").textContent=scope.length+" pools "+scopeName;$("#activeCount").textContent=scope.length+" "+scopeName
}
function render(){
