...
function summary(){
  var items=app.data.pools.map(function(p){return{pool:p,metric:metrics(p)}}),active=items.filter(function(x){return x.pool.status==="active"}).map(function(x){return x.metric}),all=items.map(function(x){return x.metric}),capital=active.reduce(function(a,m){return a+m.capital},0),current=active.reduce(function(a,m){return a+m.current},0),fees=all.reduce(function(a,m){return a+m.fees},0),variation=active.reduce(function(a,m){return a+m.variation},0),pnl=all.reduce(function(a,m){return a+m.pnl},0),pnlBase=all.reduce(function(a,m){return a+m.capital},0),eligible=all.filter(function(m){return m.apr!=null}),eligibleCapital=eligible.reduce(function(a,m){return a+m.capital},0),apr=eligibleCapital?eligible.reduce(function(a,m){return a+m.apr*m.capital},0)/eligibleCapital:null;
  $("#sumInitial").textContent=money.format(capital);$("#sumCurrent").textContent=money.format(current);$("#sumFees").textContent=money.format(fees);$("#sumVariation").textContent=signed(variation);$("#sumVariation").className=tone(variation);$("#sumVariationPct").textContent=(variation>=0?"+":"")+num.format(capital?variation/capital*100:0)+"%";$("#sumPnl").textContent=signed(pnl);$("#sumPnl").className=tone(pnl);$("#sumPnlPct").textContent=(pnl>=0?"+":"")+num.format(pnlBase?pnl/pnlBase*100:0)+"%";$("#sumApr").textContent=apr==null?"—":num.format(apr)+"%";$("#poolCount").textContent=active.length+" pools ativas";$("#activeCount").textContent=active.length+" ativas"
}
function render(){
