...
function chart(p,key){
  ...
  var feeBase=p.feesBaseline!=null?+p.feesBaseline:0;
  var vals=rows.map(function(s){var tracked=Math.max(0,(+s.fees||0)-feeBase);if(key==="fees")return tracked;if(key==="apr")return p.initialValue&&days(p,s.capturedAt||s.date)>=.9?tracked/p.initialValue*365/days(p,s.capturedAt||s.date)*100:0;return+s.value||0});
...
function card(p){
  ...
  return'<article class="pool" data-id="'+esc(p.id)+'"><div class="poolTop"><div class="pair"><div class="coin">'+esc((p.token0||"?").slice(0,2))+'</div><div><h3>'+esc(p.name)+'</h3><p>'+esc(p.exchange)+' · '+esc(p.network)+' · '+(p.source==="byreal"?"Automática":"Manual")+'</p></div></div><span class="badge '+(p.status==="closed"?"closed":inside===false?"out":"")+'">'+(p.status==="closed"?"Fechada":inside===false?"Fora da faixa":"Ativa")+'</span></div>'+
  '<div class="poolStats"><div class="poolStat"><span>Liquidez</span><strong>'+money.format(m.current)+'</strong></div><div class="poolStat"><span>'+feeLabel+'</span><strong class="positive">'+money.format(m.fees)+'</strong></div><div class="poolStat"><span title="Na Byreal, usa o PnL informado pela posição">'+pnlLabel+'</span><strong class="'+tone(m.pnl)+'">'+signed(m.pnl)+'</strong></div><div class="poolStat"><span title="Taxas acumuladas anualizadas pelo tempo desde a abertura">'+aprLabel+'</span><strong>'+(m.apr==null?"—":num.format(m.apr)+"%")+'</strong></div></div>'+ 
  ...
  '<div class="chartWrap"><div class="chartHead"><strong>Histórico · '+label+'</strong><div class="chartTabs"><button class="chartTab '+(mode==="value"?"active":"")+'" data-chart="value">Liquidez</button><button class="chartTab '+(mode==="dailyFees"?"active":"")+'" data-chart="dailyFees">Taxas 24h</button><button class="chartTab '+(mode==="fees"?"active":"")+'" data-chart="fees">Acumulado</button><button class="chartTab '+(mode==="apr"?"active":"")+'" data-chart="apr">APR</button></div></div><div class="chart">'+chart(p,mode)+'</div><div class="chartNote"><span>'+(first?when(first.capturedAt||first.date):"—")+'</span><span>'+r.length+' registros</span><span>'+(last?when(last.capturedAt||last.date):"—")+'</span></div></div>'+ 
